...
export default function HorarioPage() {
  ...
  useEffect(() => {
    ...
    if (idUsuario) {
      getWithAuth(`http://192.168.56.1:8080/cita/colaborador/${idUsuario}`)
        .then((response) => response.json())
        .then((data) => {
          const citasFiltradas = data.filter(
            (item: Cita) => item.estado === "Aceptado"
          );
  ...
  useEffect(() => {
    const fetchClientes = async () => {
      ...
      for (const id of ids) {
        const response = await getWithAuth(
          `http://192.168.56.1:8080/cliente/${id}`
        );
    ...
    const fetchPaquetes = async () => {
      ...
      for (const id of ids) {
        const response = await getWithAuth(
          `http://192.168.56.1:8080/servicio/paquete/${id}`
        );
        const data = await response.json();
    ...
    const fetchColaboradores = async () => {
      ...
      for (const id of ids) {
        const response = await getWithAuth(
          `http://192.168.56.1:8080/colaborador/${id}`
        );
        const data = await response.json();
    ...
    const fetchMotivos = async () => {
      const response = await getWithAuth(
        `http://192.168.56.1:8080/motivocancelacion`
      );
      const data = await response.json();
                ...
                <TableRow key={item.idHorario}>
                  {(columnKey) => (
                    <TableCell>
                      {columnKey === "numeroDia" ? (
                        diasSemana[
                          parseInt(item.numeroDia) as keyof typeof diasSemana
                        ] || "Desconocido"
                        ...
                        <Button
                          onClick={() =>
                            handleShowCitas(parseInt(item.numeroDia))
                          }
                        >
                          <Eye className="mr-2" />
                        </Button>
                  ...
                  >
                    {selectedCitas.map((cita) => (
                      ...
                      >
                        ...
                        <p>
                          <strong>Cliente:</strong> {clientes[cita.idCliente]}
                        </p>
                        <p>
                          <strong>Paquete:</strong> {paquetes[cita.idPaquete]}
                        </p>
                        <p>
                          <strong>Detalle:</strong> {cita.detalle || "N/A"}
                        </p>
                        <p>
                          <strong>Motivo:</strong>{" "}
                          {cita.idMotivo !== null
                            ? motivos[cita.idMotivo]
                            : "N/A"}
                        </p>
                        <p>
