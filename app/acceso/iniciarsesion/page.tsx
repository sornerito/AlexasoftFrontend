...
export default function RegistroPage() {
  ...
  const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure(); //Hook de enviar correo
  const {
    isOpen: isOpenExito,
    onOpen: onOpenExito,
  ...
  const errors = React.useMemo(() => {
    return {
      ...
      correoRecuperar:
        correoRecuperar !== "" && !validarCorreo(correoRecuperar),
      contrasenaCliente:
        contrasenaCliente !== "" && !validarContrasena(contrasenaCliente),
      contrasenaRecuperar:
        contrasenaRecuperar !== "" && !validarContrasena(contrasenaRecuperar),
    };
  }, [correoUsuario, contrasenaCliente, correoRecuperar, contrasenaRecuperar]); //Fin Validación en tiempo real
  ...
  const etapaRecuperarContraseña =
    !correoEnviado && !codigoValidado ? (
      ...
      <>
        <ModalHeader className="flex flex-col gap-1">
          Ingresa el código enviado al correo <b>{correoRecuperar}</b>
        </ModalHeader>
        <ModalBody>
          <Input
            isRequired
  ...
  const etapaBotonRecuperarContrasena =
    !codigoValidado && !correoEnviado ? (
      <Button
        className="bg-[#609448]"
        isLoading={solicitandoCorreo ? true : false}
        ...
        <div className="flex h-screen">
          ...
          <div className="items-center justify-center hidden w-1/2  sm:flex">
            <Image
              removeWrapper
              radius="lg"
              src="https://cdn.pixabay.com/photo/2022/05/10/10/19/woman-7186660_1280.jpg"
              alt="Imagen de registro"
              className="object-cover w-full h-full rounded-tl-none rounded-bl-none "
            />
          </div>
        </div>
      ) : (
            ...
            <>
              ...
              <ModalFooter>
                <Button
                  ...
                  variant="light"
                  onPress={onClose}
                  onClick={reestablecerRecuperarContrasena}
                >
                  Cancelar
                </Button>
