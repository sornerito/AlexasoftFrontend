...
import { getWithAuth, deleteWithAuth, postWithAuth, verificarAccesoPorPermiso } from "@/config/peticionesConfig";
import {
  Table,
  TableHeader,
  TableColumn,
...
import { PlusIcon, FileBarChart2, Ellipsis, Edit, Trash2Icon, CircleHelp, CircleX } from "lucide-react";
...
export default function ColaboradorPage() {
  ...
  React.useEffect(() => {
    getWithAuth("http://localhost:8080/colaborador")
      ...
      .then((data) => {
        const processedData = data.map((item: { idColaborador: any; nombre: any; cedula: any; correo: any; telefono: any; contrasena: any; estado: any; idRol: any; }) => ({
  ...
  const handleDelete = async (idColaborador: string) => {
    try {
      const response = await deleteWithAuth(`http://localhost:8080/colaborador/${idColaborador}`);
      if (response.ok) {
        setColaborador(colaborador.filter((col) => col.idColaborador !== idColaborador));
      } else {
  ...
  const handleChangeEstado = async () => {
    ...
    try {
      const response = await postWithAuth(`http://localhost:8080/colaborador/${selectedColaborador.idColaborador}/estado`, { estado: selectedEstado });
      if (response.ok) {
        setColaborador(colaborador.map(col => col.idColaborador === selectedColaborador.idColaborador ? { ...col, estado: selectedEstado } : col));
        onCloseEstado();
      } else {
  ...
  const openEstadoModal = (colaborador: Colaborador) => {
    setSelectedColaborador(colaborador);
    setSelectedEstado(colaborador.estado === "Activo" ? "Desactivado" : "Activo");
    onOpenEstado();
  };
...
{acceso ? (
    <div>
      ...
      >
        <TableHeader columns={columns.filter(column => column.uid !== "idRol")}>
          {(column) => (
        ...
        <TableBody items={items}>
          {items.map((item: Colaborador) => (
            <TableRow key={item.idColaborador}>
              {columns.filter(column => column.uid !== "idRol").map((column) => (
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          ...
          <ModalFooter>
            <Button color="warning" variant="light" onClick={onClose}>Cancelar</Button>
            <Button color="danger" variant="light" onClick={() => handleDelete(selectedColaborador!.idColaborador)}>Eliminar</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
      ...
      <Modal isOpen={isOpenEstado} onClose={onCloseEstado}>
        <ModalContent>
          ...
          <ModalBody className="text-center">
            <h1 className="text-3xl">¿Desea cambiar el estado del colaborador?</h1>
            <p>El estado se cambiará a {selectedEstado}.</p>
          </ModalBody>
          <ModalFooter>
            <Button color="warning" variant="light" onClick={onCloseEstado}>Cancelar</Button>
            <Button color="danger" variant="light" onClick={handleChangeEstado}>Cambiar Estado</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
