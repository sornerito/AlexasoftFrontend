...
export default function SalidaProductosPage() {
  ...
  useEffect(() => {
    ...
    const fetchSalidaProductos = async () => {
      try {
        const response = await getWithAuth("http://10.170.83.243:8080/compras/salidas-producto", );
  ...
  const handleSave = async () => {
    if (selectedSalidaProducto && validarMotivoAnular()) {
      ...
      try {
        const response = await postWithAuth(`http://10.170.83.243:8080/compras/salidas-producto/${selectedSalidaProducto.idSalidaProducto}`, 
