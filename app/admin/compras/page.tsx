...
export default function ComprasPage() {
  ...
  const [proveedores, setProveedores] = useState<Map<string, string>>(
    new Map()
  );
  const {
  ...
  const fetchCompraDetalles = async (idCompra: string) => {
    try {
      ...
      const compraResponse = await getWithAuth(
        `http://192.168.56.1:8080/compras/detalle-producto-compra/${idCompra}`
      );
      ...
      const productosFiltrados = productosData.filter(
        (detalle: any) => detalle.idCompra === idCompra
      );

      // Mapeamos las unidades por idProducto para la compra seleccionada
      const unidadesPorProducto: any = productosFiltrados.reduce(
        (acc: any, detalle: any) => {
      ...
      const productosAgrupados: any[] = updatedProductos.reduce(
        (acc: any, producto: any) => {
          if (!acc[producto.idProducto]) {
            acc[producto.idProducto] = {
              ...producto,
              cantidad: producto.unidadesVendidas,
            };
          }
          return acc;
        },
      ...
      setMensajeError(
        "Error al obtener Detalle de compras por producto. Por favor, inténtalo de nuevo."
      );
      onOpenError();
    }
  };
  ...
  useEffect(() => {
    const fetchProveedores = async () => {
      try {
        ...
        console.error(
          "Error al obtener proveedores: No hay algun registro o la conexion esta dañada",
          err
        );
        setMensajeError(
          "Error al obtener proveedores. Por favor, inténtalo de nuevo."
        );
        onOpenError();
      }
    };
  ...
  useEffect(() => {
    const fetchCompras = async () => {
      try {
        const response = await getWithAuth("http://192.168.56.1:8080/compras");
        const data = await response.json();
        setCompras(
          data.map((item: any) => {
        ...
        if (err.message === "Unexpected end of JSON input") {
          ...
          setMensajeError(
            "Error al obtener Compras de producto. Por favor, inténtalo de nuevo."
          );
          onOpenError();
        }
      }
  ...
  const MotivoAnular = [
    {
      key: "Eleccion de producto erronea",
      label: "Eleccion de producto erronea",
    },
    ...
    if (selectedCompra && validarMotivoAnular()) {
      ...
      try {
        const response = await postWithAuth(
          `http://192.168.56.1:8080/compras/${selectedCompra.idCompra}`,
                    ...
                    {columns.map((column) => (
                      <div key={column.uid}>
                        ...
                        {column.uid === "acciones" ? (
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownItem key={"detalles"}>
                                <Button
                                  className="w-full bg-transparent"
                                  onPress={async () => {
                                    const detalles = await fetchCompraDetalles(
                                      item.idCompra
                                    );
                          ...
                          <span>
                            {proveedores.get(item.idProveedor) ||
                              item.idProveedor}
                          </span>
                        ) : (
                      ...
                      <div className="flex flex-col gap-6 lg:flex-row">
                        ...
                        >
                          ...
                          <TableBody>
                            ...
                            <TableRow>
                              ...
                              <TableCell>
                                {proveedores.get(
                                  compraDetalles.compra.idProveedor.idProveedor
                                ) || compraDetalles.idProveedor}
                              </TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell>Motivo Anular</TableCell>
                              <TableCell>
                                {compraDetalles.compra.motivoAnular}
                              </TableCell>
                            </TableRow>
                          </TableBody>
                        </Table>
                        ...
                        <div style={{ width: "50%" }}>
                          <Table aria-label="Productos" className="flex-1">
                            ...
                            <TableBody
                              items={agrupados.slice(
                                (productPage - 1) * rowsProductoPage,
                                productPage * rowsProductoPage
                              )}
                            >
                              {(producto: any) => (
