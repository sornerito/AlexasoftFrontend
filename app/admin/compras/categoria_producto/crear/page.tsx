...
import { Input, Button, Modal, ModalContent, ModalHeader, Link, ModalBody, ModalFooter, useDisclosure, Divider, CircularProgress } from "@nextui-org/react";
...
export default function CrearCategoriaProductoPage() {
  ...
  React.useEffect(() => {
    ...
    const [categoria, setCategoria] = React.useState<CategoriaProducto>({ nombre: "", estado: "Activo" });
    ...
    const handleConfirmSubmit = async () => {
        try {
            const response = await postWithAuth("http://localhost:8080/compras/categorias-producto/", 
                categoria
            );

            if (!response.ok) {
                const errorResponse = await response.text();
                setMensajeError("No se puede crear el proveedor ya existe un nombre de la categoria ");
                onOpenError();
                    ...
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody className="text-center">
                                <h1 className=" text-3xl">¿Desea crear la categoría de producto?</h1>
                                <p>La categoría de producto se creará con la información proporcionada.</p>
                            </ModalBody>
                            <ModalFooter>
