...
export default function CategoriasProductoPage() {
  ...
  useEffect(() => {
    const fetchCategoriasProducto = async () => {
      try {
        const response = await getWithAuth("http://192.168.56.1:8080/compras/categorias-producto");
  ...
  const handleToggleEstado = React.useCallback((idCategoriaProducto: string) => {
    ...
    const promise = new Promise<void>((resolve, reject) => {
      setTimeout(() => {
        postWithAuth(`http://192.168.56.1:8080/compras/categorias-producto/${idCategoriaProducto}`,
          updatedCategoria
        ).then(response => {
          if (response.ok) {
  ...
  const handleDeleteCategoriaProductoModalConfirm = () => {
    deleteWithAuth(`http://192.168.56.1:8080/compras/categorias-producto/eliminar/${selectedCategoriaProductoId}`
    )
      .then((response) => {
        if (!response.ok) {
