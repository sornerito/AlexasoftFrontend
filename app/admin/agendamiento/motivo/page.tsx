...
export default function MotivoCancelacionPage() {
  ...
  const [selectedMotivo, setSelectedMotivo] = React.useState<Motivo | null>(
    null
  );
  const {
    isOpen: isOpenEstado,
  ...
  const handleChangeEstado = async () => {
    ...
    try {
      ...
      const response = await postWithAuth(
        `http://localhost:8080/motivocancelacion/${selectedMotivo.idMotivo}`,
        updatedMotivo
      );
      ...
      if (response.ok) {
        setMotivos(
          motivos.map((motivo) =>
            motivo.idMotivo === selectedMotivo.idMotivo ? updatedMotivo : motivo
          )
        );
                      ...
                      <TableCell key={column.uid}>
                        {column.uid === "estado" ? (
                          ...
                          <Dropdown>
                            ...
                            >
                              <DropdownItem
                                href={`/admin/agendamiento/motivo/editar/${item.idMotivo}`}
                              >
            ...
            <ModalContent>
              ...
              <ModalBody className="text-center">
                <h1 className="text-3xl">
                  ¿Desea cambiar el estado del motivo?
                </h1>
