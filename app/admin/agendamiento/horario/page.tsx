...
export default function HorarioPage() {
    ...
    const handleChangeEstado = async () => {
        ...
        try {
            const response = await postWithAuth(`http://localhost:8080/horario/${selectedHorario.idHorario}/estado`, { estado: selectedEstado });

            const data = await response.json();

            if (response.ok) {
                setHorario(horario.map(hora =>
                    hora.idHorario === selectedHorario.idHorario ? { ...hora, estado: selectedEstado } : hora
                ));
                onCloseEstado();
            } else {
                setMensajeError(data.message || "Error al cambiar el estado del horario");
                onOpenError();
            }
        } catch (error) {
                    ...
                    <Modal isOpen={isOpenEstado} onClose={onCloseEstado}>
                        <ModalContent>
                            ...
                            <ModalBody className="text-center">
                                <h1 className="text-3xl">¿Desea cambiar el estado del horario?</h1>
