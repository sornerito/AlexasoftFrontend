...
export default function EditarProveedorPage({
  ...
  React.useEffect(() => {
    ...
    useEffect(() => {
        getWithAuth(`http://localhost:8080/compras/proveedores/${params.idProveedor}`)
    ...
    const validarCampos = () => {
        ...
        if (!/^[A-Za-záéíóúÁÉÍÓÚ\s]+$/.test(nombre)) {
            erroresTemp.nombre = "El nombre no puede contener números ni caracteres especiales.";
        ...
        if (!tipoEmpresa) erroresTemp.tipoEmpresa = "El tipo de empresa es requerido.";
        if (!numeroIdentificacion) erroresTemp.numeroIdentificacion = "El número de identificación es requerido.";

        const errorNumeroIdentificacion = validarNumeroIdentificacion(numeroIdentificacion);
        if (errorNumeroIdentificacion) erroresTemp.numeroIdentificacion = errorNumeroIdentificacion;
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        switch (name) {
            ...
            case "numeroIdentificacion":
                ...
                setErrores({ ...errores, numeroIdentificacion: validarCampos().numeroIdentificacion });
                break;
    ...
    const handleConfirmSubmit = async () => {
        ...
        try {
            const response = await postWithAuth(`http://localhost:8080/compras/proveedores/editar/${params.idProveedor}`, proveedorActualizado
            );
            if (response.ok) {
                ...
                setMensajeError("No se puede editar el proveedor. Verifique los datos.");
                onOpenError();
            }
        } catch (error) {
