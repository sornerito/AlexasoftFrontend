...
export default function EditarUsuarioPage({
  ...
  React.useEffect(() => {
    getWithAuth("http://localhost:8080/usuario/" + params.idUsuario)
      .then((response) => response.json())
      .then((data) => {
        // Procesar los datos para que coincidan con la estructura de columnas
  ...
  const guardarUsuario = async () => {
    ...
    let url = "http://localhost:8080/usuario/" + params.idUsuario;
    let dataFinal = {};
    dataFinal = data;
    if (
      idRolUsuario == roles.find((rol) => rol.nombre == "Colaborador")?.idRol
    ) {
      url = "http://localhost:8080/usuario/cambiarRol/" + params.idUsuario;
